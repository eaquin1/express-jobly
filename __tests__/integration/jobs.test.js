...
describe("POST new jobs", function () {
    it("Creates a new job", async function () {
        ...
        const walker = { title: "dog walker", company_handle: 'rithm', salary: 25000, equity: 0.1, _token: TEST_DATA.userToken};
        ...
        expect(companyQuery.rows[0]).toEqual(
            {"company_handle": "rithm", "title": "dog walker"});
    });

    it("returns 400 if title is not submitted", async function () {
        const resp = await request(app)
            .post("/jobs")
            .send({ salary:99999, equity: 0.2, company_handle: TEST_DATA.currentCompany.handle, _token: TEST_DATA.userToken});
...
describe("GET all jobs", async function () {
    it("Gets a list of jobs", async function () {
        ...
        const resp = await request(app).get("/jobs").send({_token: TEST_DATA.userToken });
    ...
    it("uses search parameters", async function () {
        const resp = await request(app).get("/jobs?search=engineer").send({_token: TEST_DATA.userToken });
        expect(resp.statusCode).toEqual(200);
        expect(resp.body).toEqual({
            jobs: [{title: "Software Engineer", id: TEST_DATA.jobId, company_handle: "rithm"}],
        });
    });

    it("uses min salary search parameter", async function () {
        const resp = await request(app).get("/jobs?min_salary=49000").send({_token: TEST_DATA.userToken });
        expect(resp.body).toEqual({
            jobs: [
                {title: "Software Engineer", id: TEST_DATA.jobId, company_handle: "rithm"}
            ],
...
describe("PATCH job", function () {
    it("updates a job", async function () {
        const resp = await request(app).patch(`/jobs/${TEST_DATA.jobId}`).send({
            title: "coffee wizard", _token: TEST_DATA.userToken
        });
    ...
    it("returns 400 for bad update", async function() {
        const resp = await request(app).patch('/jobs/apple').send({
            title: 10395, _token: TEST_DATA.userToken
        })
        expect(resp.statusCode).toBe(400)
    })
    ...
    it("responds with a 404 when job not found", async function() {
        const resp = await request(app).patch('/jobs/0').send({
            title: "robot", _token: TEST_DATA.userToken
        })
...
describe("DELETE job", function(){
    it("deletes a job", async function() {
        const resp = await request(app).delete(`/jobs/${TEST_DATA.jobId}`).send({_token: TEST_DATA.userToken})
        expect(resp.statusCode).toBe(200)
    })
})
